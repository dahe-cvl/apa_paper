...
class ExtendedFeatureDataset(ClassificationDataset):
	# The Face dataset.
	nSamples = 0;
	nSamplesTrain = 0;
	nSamplesVal = 0;
	...
	def sample(self, sid):
		...
		if(self.split == "train" and sid < self.nSamples and sid >= 0):	
			...
			resized_image = cv2.resize(rgb_frame, (sample.shape[0]/2, sample.shape[1]/2)) 
	...
	def getAllSamplesOfID(self, vid):
		...
		for i in range(0, foundIdx.shape[1], 1):
			sample = self.database["data"][foundIdx[0][i]]
			resized_image = cv2.resize(sample, (sample.shape[0]/2, sample.shape[1]/2)) 
